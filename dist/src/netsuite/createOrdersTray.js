...
define(['N/record', 'N/search'], function (record, search) {
     ...
     * @param context
     */
    var searchColumns = ['entityid', 'internalid', 'custentity_enl_cnpjcpf'];
    /*
    ...
    function createOrder(context) {
        try {
            ...
            var salesOrder_1 = record.create({
            ...
            salesOrder_1.setValue({ fieldId: 'custbody_udf_tray_order_id', value: context.id });
            salesOrder_1.setValue({ fieldId: 'trandate', value: new Date() });
            salesOrder_1.setValue({ fieldId: 'department', value: '3' });
            salesOrder_1.setValue({ fieldId: 'class', value: '1' });
            salesOrder_1.setValue({ fieldId: 'location', value: '1' });
            if (context.payment_method === 'Boleto - Yapay') {
                salesOrder_1.setValue({ fieldId: 'terms', value: '18' });
            }
            else {
                if (context.installment === 1) {
                    salesOrder_1.setValue({ fieldId: 'terms', value: '20' });
                }
                if (context.installment === 2) {
                    salesOrder_1.setValue({ fieldId: 'terms', value: '24' });
                }
                if (context.installment === 3) {
                    salesOrder_1.setValue({ fieldId: 'terms', value: '25' });
                }
            ...
            salesOrder_1.setValue({ fieldId: 'custbody_enl_freighttype', value: '1' });
            if (context.shipment === 'Frete grátis') {
            ...
            else if (context.shipment === 'Expresso') {
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3670,
                });
            ...
            else if (context.shipment === 'Braspress') {
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3653,
                });
            ...
            else if (context.shipment === 'Braspress Multiply') {
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3652,
                });
            ...
            else if (context.shipment === 'Correios Sedex 10') {
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3651,
                });
            ...
            else if (context.shipment === 'Braspress Aéreo') {
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3654,
                });
            ...
            else if (context.shipment === 'Araçalog Aéreo') {
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3656,
                });
            ...
            else if (context.shipment === 'Aracalog Standard') {
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3657,
                });
            ...
            else if (context.shipment === 'Retirar na UDF') {
                ...
                salesOrder_1.setValue({ fieldId: 'custbody_enl_freighttype', value: '10' });
                salesOrder_1.setValue({
                    fieldId: 'shipmethod',
                    value: 3686,
                });
            ...
            var shippingAddress = salesOrder_1.getSubrecord({
            ...
            salesOrder_1.selectNewLine({ sublistId: 'item' });
            var products = context.products;
            products.forEach(function (product) {
                salesOrder_1.setCurrentSublistValue({
                    sublistId: 'item',
                    fieldId: 'item',
                ...
                salesOrder_1.commitLine({ sublistId: 'item' });
            });
            var salesOrderId = salesOrder_1.save({
                ignoreMandatoryFields: false,
                enableSourcing: false,
            });
            return salesOrderId;
    ...
    function store(context) {
        try {
            var orders = context.completedOrders;
            var createdOrders = orders.map(function (order) {
                ...
                if (customers.length > 0) {
                    ...
                    var customerId = createCustomer(order.customer);
